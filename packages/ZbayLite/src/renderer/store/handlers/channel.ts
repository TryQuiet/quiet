...
const isSizeCheckingInProgress = createAction<boolean>(
  actionTypes.IS_SIZE_CHECKING_IN_PROGRESS
);
const messageSizeStatus = createAction<boolean>(
  actionTypes.MESSAGE_SIZE_STATUS
);
const setShareableUri = createAction<string>(
  actionTypes.SET_CHANNEL_SHAREABLE_URI
);
const setDisplayableLimit = createAction<number>(
  actionTypes.SET_DISPLAYABLE_LIMIT
);
...
const loadChannel = (key) => async (dispatch, getState) => {
  try {
    ...
    dispatch(
      contactsHandlers.actions.cleanNewMessages({ contactAddress: key })
    );
...
export const reducer = handleActions<ChannelStore, PayloadType<ChannelActions>>(
  {
    [setLoading.toString()]: (
      state,
      { payload: loading }: ChannelActions["setLoading"]
    ) =>
      produce(state, (draft) => {
    ...
    [setMessage.toString()]: (
      state,
      { payload: value }: ChannelActions["setMessage"]
    ) =>
      produce(state, (draft) => {
        draft.message[draft.id] = value;
      }),
    [setChannelId.toString()]: (
      state,
      { payload: id }: ChannelActions["setChannelId"]
    ) =>
      produce(state, (draft) => {
        draft.id = id;
      }),
    ...
    [messageSizeStatus.toString()]: (
      state,
      { payload }: ChannelActions["messageSizeStatus"]
    ) =>
      produce(state, (draft) => {
        draft.messageSizeStatus = payload;
      }),
    [setShareableUri.toString()]: (
      state,
      { payload: uri }: ChannelActions["setShareableUri"]
    ) =>
      produce(state, (draft) => {
    ...
    [setAddress.toString()]: (
      state,
      { payload: address }: ChannelActions["setAddress"]
    ) =>
      produce(state, (draft) => {
