...
describe('local transport gating', () => {
  ...
  describe('when enabled', () => {
    ...
    it('round trips a local dial multiaddr through the invitation link helpers', () => {
      expect(getAddressFromLibp2pAddress(LOCAL_MULTIADDR)).toBe('127.0.0.1:45321')
      expect(p2pAddressesToPairs([LOCAL_MULTIADDR])).toEqual([{ peerId: PEER_ID, onionAddress: '127.0.0.1:45321' }])
      expect(pairsToP2pAddresses([{ peerId: PEER_ID, onionAddress: '127.0.0.1:45321' }])).toEqual([LOCAL_MULTIADDR])
  ...
  describe('when not fully enabled', () => {
    it.each([
