...
const validateInvitationKind: InvitationLinkUrlNamedParamValidatorFun<InvitationData> = (
  ...
  return { kind: value } as Partial<InvitationData>
}
...
export const PARAM_CONFIG_V4: VersionedInvitationLinkUrlParamConfig<InvitationDataV4 | DeviceInvitationDataV4> = {
  ...
  named: new Map(
    Object.entries({
      ...
      [INVITATION_KIND_KEY]: { required: false, validator: validateInvitationKind },
      [AUTH_DATA_KEY]: {
        required: true,
        ...
        nested: {
          ...
          config: new Map(
            Object.entries({
              ...
              [USER_ID_KEY]: { required: false, validator: validateUserId },
              [USER_NAME_KEY]: { required: false, validator: validateUserName },
            })
          ),
        },
      },
...
export const parseAndValidateUrlParams = <T extends InvitationData>(
  ...
  if (invitationData.kind === InvitationKind.Device) {
    ...
    if ('salt' in authData) throw new UrlParamValidatorError(`${AUTH_DATA_KEY}.${SALT_KEY}`, authData.salt)
  } else {
    if (authData.userId != null) throw new UrlParamValidatorError(`${AUTH_DATA_KEY}.${USER_ID_KEY}`, authData.userId)
    if (authData.userName != null)
