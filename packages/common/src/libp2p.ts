...
/**
 * Whether the non-Tor local transport is available.
 *
 * Both flags are required so the local path is unreachable in production: a
 * shipped build never sets IS_E2E, so LOCAL_TRANSPORT alone cannot open it.
 * Every local branch below funnels through this, which keeps address creation,
 * listen addresses, invite-link parsing and transport selection consistent.
 *
 * @returns {boolean} `true` when local transport is enabled for this process
 */
...
/**
 * Parses a `127.0.0.1:<port>` local transport address
 * @param address - The address to parse
 * @returns The parsed local address, or undefined if it is not one
 */
...
/**
 * Creates a `127.0.0.1:<port>` local transport address
 * @param port - The port libp2p listens on
...
/**
 * Extracts the peer address from a libp2p multiaddr.
 *
 * Invite links carry `<peerId>,<address>` pairs rather than multiaddrs, so the
 * multiaddr has to collapse back to the address form the link stores. Onion
 * multiaddrs use `/dns4/`; local transport multiaddrs use `/ip4/` and need the
 * port preserved, which the onion form never carries.
 *
 * @param peerAddress - The libp2p multiaddr
 * @returns The peer address, or undefined if it cannot be extracted
 */
