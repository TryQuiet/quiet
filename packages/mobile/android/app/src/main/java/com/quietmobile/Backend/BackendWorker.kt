...
class BackendWorker(private val context: Context, workerParams: WorkerParameters) : CoroutineWorker(context, workerParams) {
    ...
    override suspend fun doWork(): Result {
        ...
        withContext(Dispatchers.IO) {
            ...
            launch {
                delay(6000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message1\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message2\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message3\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message4\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message5\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message6\"}}")
                delay(6000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message7\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message8\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message9\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"message\", \"payload\": {\"event\": \"message10\"}}")
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"close\", \"payload\": {\"event\": \"messageClose\"}}")
                val dataPort = Utils.getOpenPort(11233)
                startWebsocketConnection(dataPort)
                delay(5000)
                sendMessageToNodeChannel("_EVENTS_", "{ \"event\": \"open\", \"payload\": {\"socketIOPort\": $dataPort}}")
            }
