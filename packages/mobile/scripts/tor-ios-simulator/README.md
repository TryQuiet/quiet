...
The manual workflow [`.github/workflows/e2e-ios.yml`](../../../../.github/workflows/e2e-ios.yml) prepares this pinned Tor framework before invoking `detox build -c ios.sim.e2e`. It uses the ARM `macos-15-xlarge` runner, explicitly selects Xcode 26.3, and creates one iPhone 16 Pro on the installed iOS 18.5 runtime. Those tools are listed in GitHub's [macOS 15 ARM image manifest](https://github.com/actions/runner-images/blob/main/images/macos/macos-15-arm64-Readme.md); the [larger runner labels](https://docs.github.com/en/actions/reference/runners/larger-runners#available-macos-larger-runners-and-labels) identify `macos-15-xlarge` as ARM. Node comes from `.nvmrc`; Python 3.12 and the locked Ruby/Bundler/CocoaPods dependencies are selected explicitly. Checkout's submodule commits are preserved. The bundled E2E app runs the starter suite and message acknowledgment/persistence test without Metro, with the same framework, app path and exact simulator ID throughout. An empty temporary Firebase plist permits the simulator build without push service credentials; it is removed afterward. Cleanup deletes only the simulator created by this run. Tor/build logs, per-build results, Detox artifacts and a final screenshot are retained for seven days. Hosted execution still needs publication of the authentication submodule commit and the workflow change; local validation does not establish a hosted CI pass.

The workflow's portable regression tests execute its actual shell steps against a disposable simulator-command recorder, exercise the installed Detox CLI and real configuration with a recording builder, and verify temporary plist creation/cleanup with Python. They cover failed simulator creation/boot, exact device cleanup, build-failure propagation and the E2E framework/output handoff. They do not simulate a passing native build or runtime test:
