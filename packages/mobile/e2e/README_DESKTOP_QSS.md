...
Android shutdown cancels JobScheduler work for only the selected app/user before terminating Detox instrumentation, then issues a final `am force-stop`. It requires an empty app UID process set and `stopped=true` package state to remain stable for 1.5 seconds. This prevents queued WorkManager services from reviving the app during an apparent process gap. Later offline checks fail immediately if the package has restarted, even if its process subsequently exited. Normal app launch re-enqueues the backend worker; app data is retained for the persistence checks. The proof records the scoped job cancellation and stopped package state.
