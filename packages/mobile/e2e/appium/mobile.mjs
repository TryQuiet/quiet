...
export class Mobile {
  ...
  id(value) {
    return this.android ? `//*[@resource-id=${literal(value)}]` : `~${value}`
  }
  text(value) {
    return this.android ? `//*[@text=${literal(value)}]` : `//*[@label=${literal(value)}]`
  }
  ...
  async tapText(value, timeout) {
    await (await this.visible(this.text(value), timeout)).click()
  }
  async tapId(value) {
    await (await this.visible(this.id(value))).click()
  }
  async adb(...args) {
    const sdk = process.env.ANDROID_HOME || process.env.ANDROID_SDK_ROOT
    return exec(
      path.join(sdk, 'platform-tools/adb'),
      ['-P', String(this.config.adbPort || 5037), '-s', this.config.udid, ...args],
  ...
  async start() {
    ...
    this.driver = await remote({
      hostname: '127.0.0.1',
      port: this.config.appiumPort || 4725,
      logLevel: 'silent',
  ...
  async allowNotifications() {
    if (this.android) {
      ...
      if (await allow.isExisting()) {
        const message = await this.driver.$(
  ...
  async background() {
    ...
    if (this.android) {
      ...
      assert.match(
        ...
        'Notification permission must be granted through the OS'
      )
    ...
    await this.driver.waitUntil(async () => (await this.driver.queryAppState(this.config.bundleId)) !== 4, {
      timeout: 15000,
      timeoutMsg: 'Quiet remained in foreground',
    })
  }
  ...
  async notificationAndTap({ text, username, channel, artifact }) {
    ...
    if (this.android) {
      ...
      const row = await this.driver.$(
        `//*[@resource-id="com.android.systemui:id/expandableNotificationRow"][.//*[@package="com.android.systemui" and @text=${literal(body)}]]`
      )
      assert(await row.isExisting(), 'Expected a real System UI notification row')
      assert(
        await row.$(`.//*[@text=${literal(channel)}]`).isExisting(),
        'Notification must display the human channel name'
      )
      const { stdout } = await this.adb(
        ...
        'epoch',
        '-T',
        `${this.backgroundEpoch}.000`,
        '-s',
        'QssFirebaseMessaging:I'
      )
      ...
      assert(
        stdout.includes(`entries for teamId=${this.teamId}`),
        "The native receiver must fetch this run's authenticated QSS team"
      )
      assert.match(
        stdout,
        /Posting notification for cid=/,
        'The production QSS push handler must authenticate and post the notification'
      )
    } else {
      const title = `${username} in #${channel}`
      const container = await this.driver.$(
        '//*[contains(@name, ' + literal(body) + ') and contains(@name, ' + literal(title) + ')]'
      )
      assert(
        await container.isExisting(),
        'The same Notification Center item must display the exact sender, channel, and body'
      )
    }
    ...
    await this.driver.waitUntil(async () => (await this.driver.queryAppState(this.config.bundleId)) === 4, {
      timeout: 30000,
      timeoutMsg: 'Tapping the OS notification did not open Quiet',
    })
    await this.message(text, username, channel)
    await this.artifacts(`${artifact}-opened`)
    return {
      provider: this.android ? 'FCM' : 'FCM/APNs',
      osPresentation: true,
      tapped: true,
      channel,
      text,
      username,
      activationEvidence: this.android
        ? 'Firebase service receive and QSS handler post logs'
        : 'Decrypted unique peer text in OS notification; QPS provider payload contains only generic text',
    }
  }
  async artifacts(name) {
    ...
    await fs.writeFile(
      path.join(this.run.directory, `${name}.png`),
