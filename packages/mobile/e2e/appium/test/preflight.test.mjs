...
test('iOS inspects binary bundle plists and refuses stale native or extension configuration', { skip: process.platform !== 'darwin' }, async t => {
  ...
  assert.throws(() => inspectIosBuild(config, false), /selected QSS test endpoint/)
  plist('Env.plist', env)
  ...
  const staging = 'wss://qss-dev.quiet-services.app'
  plist('Env.plist', { ...env, QPS_ALLOWED: 'true', QSS_ENDPOINT: staging })
  fs.writeFileSync(path.join(app, 'Quiet.debug.dylib'), staging)
  assert.equal(inspectIosBuild(config, true, staging).extensionSHA256, implementationHash)
  assert.throws(() => inspectIosBuild(config, true), /selected QSS test endpoint/)
  fs.writeFileSync(path.join(app, 'Quiet.debug.dylib'), staging + ' wss://qss-prod.quiet-services.app')
  assert.throws(() => inspectIosBuild(config, true, staging), /different QSS endpoint/)
  plist('Env.plist', { ...env, QPS_ALLOWED: 'true' })
  fs.writeFileSync(path.join(app, 'Quiet.debug.dylib'), env.QSS_ENDPOINT)
