...
// Run with android.att.storybook; no backend or Node-process substitute runs.
const describeAndroid = device.getPlatform() === 'android' ? describe : describe.skip

describeAndroid('React Native runtime compatibility', () => {
  beforeEach(async () => {
    ...
    await waitFor(element(by.id('runtime-compatibility-run'))).toBeVisible().withTimeout(10000)
  })
  ...
  it('uses Hermes and the real WebView bridge to hash, round-trip keys, sign, and reject tampering', async () => {
    ...
    await waitFor(element(by.id('runtime-compatibility-state'))).toHaveText('passed').withTimeout(30000)
