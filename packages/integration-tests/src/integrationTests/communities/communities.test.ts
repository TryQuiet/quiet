...
import { createUserCsr, UserCsr, configCrypto } from '@zbayapp/identity'
import {
  Community, CommunityChannels, Identity, StoreKeys, identityAdapter, communitiesAdapter, communities, identity, publicChannels,
...
describe('communities - with tor', () => {
  ...
  test('Owner and users received certificates', async () => {
    await assertReceivedCertificates('owner', 3, owner.store, 120_000)
    await assertReceivedCertificates('userOne', 3, userOne.store, 120_000)
    await assertReceivedCertificates('userTwo', 3, userTwo.store, 120_000)
  })

  test('Users replicated channel and subscribed to it', async () => {
    await assertReceivedChannelsAndSubscribe('owner', 1, owner.store, 120_000)
  ...
  test('Every user replicated all messages', async () => {
    await assertReceivedMessages('owner', 3, owner.store, 120_000)
    await assertReceivedMessages('userOne', 3, userOne.store, 120_000)
    await assertReceivedMessages('userTwo', 3, userTwo.store, 120_000)
  })
