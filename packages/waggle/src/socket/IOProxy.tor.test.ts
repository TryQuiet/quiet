...
test('IO proxy closes all services (using tor)', async () => {
  ...
  const hiddenService2 = {
    address: 'uec6jplmrnx3mocxtq3uc52cxlfdoq3ln4gqa4vkxapn74ro4glyfsqd.onion',
    privateKey:
      'ED25519-V3:oDVdj31fEb1v8j7YIBA+Ohnnmg9BIt+3OLC7AXFWw34Wwc8mgTeNd66FLdKCv4vfCA1Mumele58HPd8xgIED9g=='
  }
  const pems = await createCertificatesTestHelper(hiddenService1.address, 'adres2.onion')
  const certs1 = {
    cert: pems.servCert,
    key: pems.servKey,
    ca: [pems.ca]
  ...
  await ioProxy.createCommunity('myCommunity1', peerId1, hiddenService1, certs1)
  await ioProxy.launchRegistrar('myCommunity1', peerId1.id, pems.ca, pems.ca_key)
  await ioProxy.createCommunity('myCommunity2', peerId2, hiddenService2, certs2)
