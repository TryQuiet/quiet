...
import { InvitationData, InvitationDataVersion, JoinCommunityPayload, isDeviceInvitationData } from '@quiet/types'
...
export function* customProtocolSaga(
  ...
  if (isDeviceInvitationData(data)) {
    ...
    yield* put(communities.actions.linkDevice({ inviteData: data }))
    return
  }
