...
`MuiMenuItem` (Overlay menu), `MuiDialog` (modal/small), `MuiListItemButton` selected (Search result) and
the body text on the body role — and
...
| T7 | Secondary buttons: `DeleteChannel:68-80`, `LeaveCommunity:62-74`, `UpdateModal:58-70`, `AddMembersChannel:78-90` | a *contained* white 160×40 button in darkGray whose hover does nothing | `action bar / Buttons=Single with Cancel` `3505:10336`: Cancel `3505:10321` = white, 1px #B3B3B3 (border02), Rubik 14/20 w400, padding 6/12 | M | **fixed** — `variant='outlined'` in the four modals. The `outlined` override itself now comes from develop's shared `buttonStyleOverrides` in `theme.ts`, which carries the same 1px #B3B3B3 and puts the library's Large metrics on the button root with Small as the override; 14/20 with 6/12 padding is therefore `size='small'` on that scale, and the corner radius (16) landed with it. This branch's `buttonOutlined` helper is gone as redundant. |
...
| M18 | `Channel/AddMembersChannel/AddMembersChannelComponent.tsx:64,104-125,275-280` | `1rem`/1.68 literals, margin 5, marginTop 25, paddingLeft 2px, inline `fontWeight: 500, fontSize: 16` | bodyLg role; 4; 24; h5 | L | **superseded** — develop rebuilt this screen as a right-hand `Drawer` with `PanelHeader`, `PillField` and checkbox rows, so the modal these literals lived in no longer exists. Nothing to fix; the story title collision is also gone (develop retitled it `Private channels/3. Add members`). |
