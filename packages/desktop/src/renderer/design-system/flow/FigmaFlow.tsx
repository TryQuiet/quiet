...
export interface Shell {
  ...
  width: number
  height: number
  topBar: { y: number; h: number }
  titleBar: { y: number; h: number }
  titleZone: Rect
  backZone: Rect
  content: Rect
}
...
const navigate = (f: FlowFrame): void => {
  if (window.parent !== window) linkTo(FLOW_TITLE, f.display)()
    ...
    u.searchParams.set('id', storyIdFor(f))
...
const writeHist = (h: string[]): void => {
  try {
    sessionStorage.setItem(HIST, JSON.stringify(h.slice(-50)))
  } catch {
    /* private mode */
  }
}
    ...
    {impl.status === 'none' ? (
      ...
      <span
        style={{
          fontFamily: mono,
          fontSize: 11,
          color: impl.status === 'pr3400' ? '#8A5F09' : INK,
          wordBreak: 'break-all',
        }}
      >
        {impl.path}
        {impl.status === 'pr3400' ? '  (PR #3400, not on this branch)' : ''}
      </span>
    )}
  </div>
)
...
const writeVp = (v: Viewport): void => {
  try {
    sessionStorage.setItem(VP_KEY, v)
  } catch {
    /* private mode */
  }
}
...
const Hotspot: React.FC<{
  ...
  outline: boolean
  onClick: () => void
  describe: (l: FlowLink) => string
}> = ({ l, rect, outline, onClick, describe }) => {
  ...
  return (
    <button
      ...
      style={{
        ...
        width: rect.w,
        height: rect.h,
        background: outline ? st.bg : 'transparent',
        border: outline ? st.border : '1px solid transparent',
        borderRadius: 4,
        cursor: 'pointer',
        padding: 0,
        font: 'inherit',
...
const Stretched: React.FC<{ png: string; width: number; height: number; stretch: Stretch; winW: number }> = ({
  png,
  width,
  height,
  stretch,
  ...
  const gap = Math.max(0, winW - width)
  const rightW = width - stretch.right
  return (
    <>
      <div style={{ position: 'absolute', left: 0, top: 0, width: stretch.right, height, overflow: 'hidden' }}>
        <img src={dsrc(png)} width={width} height={height} alt='' style={{ display: 'block' }} />
      </div>
      {gap > 0 && (
        <div style={{ position: 'absolute', left: stretch.right, top: 0, width: gap, height, overflow: 'hidden' }}>
          <div style={{ width: 1, height, overflow: 'hidden', transform: `scaleX(${gap})`, transformOrigin: '0 0' }}>
            <img
              src={dsrc(png)}
              width={width}
              height={height}
              alt=''
              style={{ display: 'block', position: 'relative', left: -stretch.col }}
            />
          </div>
        </div>
      )}
      <div style={{ position: 'absolute', left: winW - rightW, top: 0, width: rightW, height, overflow: 'hidden' }}>
        <img
          src={dsrc(png)}
          width={width}
          height={height}
          alt=''
          style={{ display: 'block', position: 'relative', left: -stretch.right }}
        />
      </div>
    </>
  )
}
const shiftX = (x: number, stretch: Stretch, width: number, winW: number): number =>
  x >= stretch.right ? x + Math.max(0, winW - width) : x

/** Desktop rendering of a stage from the designer's own desktop frame (see gen.cjs DESKTOP). */
const DesktopFrameAt: React.FC<{
  frame: FlowFrame
  vp: Viewport
  outline: boolean
  follow: (l: FlowLink) => void
  describe: (l: FlowLink) => string
}> = ({ frame, vp, outline, follow, describe }) => {
      ...
      <div style={box(d.height)}>
        ...
        {d.hotspots.map((l, i) => (
          <Hotspot
            key={i}
            l={l}
            rect={{ x: shiftX(l.x, st, d.width, winW), y: l.y, w: l.w, h: l.h }}
            outline={outline}
            onClick={() => follow(l)}
            describe={describe}
          />
...
const ScreenAt: React.FC<{
  ...
  follow: (l: FlowLink) => void
  describe: (l: FlowLink) => string
}> = ({ flow, frame, vp, outline, follow, describe }) => {
  const shell = flow.shell
  if (vp !== 'mobile' && vp !== 'mobile-430' && frame.desktop && frame.desktop.kind === 'app')
    return <DesktopFrameAt frame={frame} vp={vp} outline={outline} follow={follow} describe={describe} />
  if (vp === 'mobile' || vp === 'mobile-430' || !shell) {
    ...
    return (
      <div
        style={{
          position: 'relative',
          width: Math.round(frame.width * s),
          height: Math.round(frame.height * s),
          border: `1px solid ${RULE}`,
          borderRadius: 6,
          overflow: 'hidden',
          background: '#fff',
      ...
      >
        <img
          src={src(frame.png)}
          width={Math.round((frame.width + 2 * (frame.pad?.x ?? 0)) * s)}
          height={Math.round((frame.height + 2 * (frame.pad?.y ?? 0)) * s)}
          alt={frame.name}
          style={{
            display: 'block',
            position: 'relative',
            left: -Math.round((frame.pad?.x ?? 0) * s),
            top: -Math.round((frame.pad?.y ?? 0) * s),
          }}
        />
        {frame.links.map((l, i) => (
          <Hotspot
            key={i}
            l={l}
            rect={{ x: l.x * s, y: l.y * s, w: l.w * s, h: l.h * s }}
            outline={outline}
            onClick={() => follow(l)}
            describe={describe}
          />
    ...
    >
      ...
      >
        {[0, 1, 2].map(i => (
          <span
            key={i}
            style={{ width: 12, height: 12, borderRadius: 6, background: '#C4C4C4', display: 'inline-block' }}
          />
        ))}
      </div>
      ...
      >
        <img
          ...
          alt={frame.name}
          style={{ display: 'block', transform: `translateY(-${crop}px)` }}
        />
      </div>
      {content.links.map((l, i) => {
        ...
        return (
          <Hotspot
            key={i}
            l={l}
            rect={{ x: contentLeft + l.x, y: contentTop + l.y - crop, w: l.w, h: l.h }}
            outline={outline}
            onClick={() => follow(l)}
            describe={describe}
          />
...
export const Stage: React.FC<{ flow: Flow; frame: FlowFrame }> = ({ flow, frame }) => {
  ...
  const describe = (l: FlowLink): string =>
    l.kind === 'back' ? 'back (history)' : l.target && bySlug[l.target] ? bySlug[l.target].display : String(l.target)
  ...
  return (
    ...
    >
          ...
          {VIEWPORTS.map(v => (
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                cursor: 'pointer',
                border: `1px solid ${vp === v.id ? '#0D6420' : RULE}`,
                background: vp === v.id ? '#0D642014' : '#fff',
                color: vp === v.id ? '#0D6420' : INK_2,
              }}
            >
              {v.label}
            </button>
        ...
        {isDesktop ? (
          <p
            style={{
              ...
              maxWidth: 720,
              paddingLeft: 10,
              borderLeft: '2px solid #8A5F09',
            }}
          >
            {frame.desktop ? (
              <>
                Desktop rendering from the designer&rsquo;s own desktop frame (node {frame.desktop.node},{' '}
                {frame.desktop.width}&times;{frame.desktop.height}), not the mobile content in the shell.{' '}
                {frame.desktop.kind === 'app' ? (
                  <>
                    Drawn at {frame.desktop.width}: in wider windows the sidebar and the chat&rsquo;s edges keep their
                    place and the gap is the frame&rsquo;s own plain column stretched — only designer pixels
                    (&ldquo;sidebar on left and chat in the rest of the screen&rdquo;).
                  </>
                ) : (
                  <>715-wide content inside the Modal full-window shell.</>
                )}{' '}
                Hotspots are measured on the export and wired to the same targets as the mobile frame&rsquo;s links.
              </>
            ) : (
              <>
                Desktop sizes are a composition per the decided model, not a designed screen: the Modal full-window
                shell&rsquo;s chrome (window controls, title bar with back arrow) drawn to the window width, a white
                content area, and this screen&rsquo;s content centered in it — its own title bar removed and its title
                text set in the shell&rsquo;s bar. The library component is 715 wide; stretching it to wider windows is
                our interpretation.
              </>
            )}
          </p>
        ) : vp === 'mobile-430' ? (
          <p style={{ fontSize: 12, lineHeight: '17px', color: INK_3, margin: '8px 0 0', maxWidth: 720 }}>
            Scaled from the 375-wide frame; the design has no 430-wide variant.
          </p>
        ) : null}
      </div>
      ...
      <div style={{ maxWidth: 420, minWidth: 280, flex: 1 }}>
        ...
        <h1
          style={{ fontSize: 22, lineHeight: '28px', fontWeight: 500, margin: '2px 0 4px', letterSpacing: '-0.01em' }}
        >
          {frame.display}
        </h1>
          ...
          <ul style={{ margin: '4px 0 0', padding: 0, listStyle: 'none' }}>
            {frame.links.map((l, i) => (
              <li key={i} style={{ fontSize: 13, lineHeight: '20px' }}>
                ...
                <span style={{ fontFamily: mono, fontSize: 10, color: KIND_INK[l.kind], marginLeft: 6 }}>
                  {KIND_STYLE[l.kind].word}
                </span>
        ...
        {impl ? (
          ...
          <p style={{ fontSize: 13, color: INK_3, margin: '6px 0 0' }}>
            not mapped — an intermediate prototype state (photo picker / crop / focus state)
          </p>
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
      <img
        src={src(f.png)}
        alt=''
        style={{ width: '100%', height: 'auto', display: 'block', borderRadius: 3, border: `1px solid ${RULE}` }}
      />
      ...
      <div
        style={{
          fontSize: 10.5,
          fontFamily: mono,
          color: tag === 'no implementation' ? '#A11F24' : tag === 'in PR #3400' ? '#8A5F09' : INK_3,
          marginTop: 2,
        }}
      >
        {tag}
...
const SECTION_NOTE: Record<string, string> = {
  Onboarding:
    'Reachable from "Get started" by the prototype\u2019s own links (plus one added link to the paste screen). After username the prototype lands on Community home, and from there the server / plan / captcha / subscription cluster is reachable — creator-side, once the community exists. Both apps show that offer during creation instead.',
  'Server opt-in (creator)':
...
export const FlowMap: React.FC<{ flow: Flow }> = ({ flow }) => (
  <div style={{ padding: 24, fontFamily: "'Rubik', sans-serif", color: INK, maxWidth: 900 }}>
    <h1 style={{ fontSize: 26, lineHeight: '34px', fontWeight: 500, margin: '0 0 4px' }}>
    ...
    {flow.sections
      .filter(sec => flow.frames.some(f => f.section === sec))
      .map(sec => (
        <div key={sec} style={{ marginBottom: 28 }}>
          ...
          {SECTION_NOTE[sec] ? (
            <p style={{ fontSize: 12.5, color: INK_2, margin: '0 0 10px', maxWidth: '70ch' }}>{SECTION_NOTE[sec]}</p>
          ) : null}
    ...
    >
      stages the apps have that the prototype does not draw
    </h2>
  ...
  return (
    <div style={{ padding: 24, fontFamily: "'Rubik', sans-serif", color: INK, maxWidth: 1100 }}>
      ...
      <p style={{ fontSize: 13, lineHeight: '19px', color: INK_2, margin: '0 0 6px', maxWidth: '80ch' }}>
        The Get started prototype is mobile-only, but desktop onboarding designs exist as library components and
        instances in four files. Each is the designer&rsquo;s export, with the file and its last-edit date — several
        predate the mobile prototype by over a year.
      </p>
      <p
        style={{
          fontSize: 13,
      ...
      >
        The desktop model: every desktop onboarding <em>modal</em> is the library&rsquo;s <em>Modal full-window</em>{' '}
        shell with the same 375-wide content mobile uses inside it; once the community exists the app is the split view
        — the V1 desktop sidebar on the left, the chat filling the rest (decided 2026-09-12) — the library&rsquo;s{' '}
        <em>Join community</em> variants are 375 wide. Routes agree with mobile. <em>Register username</em> is stale
        cruft; desktop username takes the mobile prototype&rsquo;s content.
      </p>
      {DESKTOP.map(d => {
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 260, maxWidth: 400 }}>
              ...
              <p style={{ fontSize: 12, color: INK_3, margin: '0 0 8px' }}>
                {d.width}×{d.height} · node <span style={{ fontFamily: mono }}>{d.node}</span> ·{' '}
                <a href={d.url} target='_blank' rel='noreferrer' style={{ color: '#0D6420' }}>
                  open in Figma
                </a>
              </p>
              ...
              {cp ? (
                <p style={{ fontSize: 13, margin: '0 0 8px' }}>
                  <span style={{ color: INK_3 }}>mobile counterpart: </span>
                  <button
                    type='button'
                    onClick={() => navigate(cp)}
                    style={{
              ...
              <ul style={{ margin: 0, padding: '0 0 0 16px', fontSize: 12.5, lineHeight: '18px', color: INK_2 }}>
                {d.copy.map((t, i) => (
                  <li key={i}>{t}</li>
                ))}
              </ul>
