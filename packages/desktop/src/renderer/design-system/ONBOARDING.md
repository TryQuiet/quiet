...
- **The Agree & join card** (`agree-and-join`, `3054:4090`) — a titled, ruled bar and a **left-aligned** block starting **16** below it, not the centred column the stages use. It is a consent card, and the left alignment is the frame's. On desktop it is drawn **full window** like the stages, not as the library's floating modal/small card (user decision, #3690: the card read as a dialog over the app); the 468 column is centred in the window. The device-link consent step shares it.
...
**User decision (2026-09-13): people display the QR code; the raw link is never shown.** Both platforms render this sheet's content (desktop inside the Link devices modal and in Settings → Linked devices, per the Device-linking desktop frame `880:17427`; mobile as a full screen — the sheet itself is a cross-cutting parity item): title bar *QR code* with close (desktop: the close glyph only, no bar title — user decision, #3690), the QR in the library's `qr-code-box` (220, 1px `#B3B3B3` r4, 188 code), the sentence below verbatim, then **Copy link** — a primary button in exactly the slot the Add members QR sheet (`2932:3707`) gives *Share code* (50 tall, r16, padding 20, hug width, centred) — and *Reset QR code* as the text link (16/26 `#2373EA`), which mints a new link; desktop does not draw Reset QR code (user decision, #3690). Desktop does not draw Copy link on the sheet either (user decision, #3690): the Link devices screen's own Copy link row copies the same link, so the sheet has no action at all. Copy link puts the link on the clipboard and confirms briefly (*Link copied*: desktop MUI Snackbar, mobile the app's confirmation box); the masked link field with the eye icon and *Copy to clipboard* are gone. Undesigned states, kept with #3400's copy: while the backend mints, the actions give way to the library's progress bar with *Generating device link…* as its status line (`ActionProgress`, #3518's rule — never a greyed-out button; the mobile port lives in `components/ActionProgress`); *Device link unavailable* in the box without a community (desktop Settings only), with no action drawn. Implemented by: desktop `Onboarding/DisplayQrCodeComponent.tsx` (+ `DisplayQrCode.tsx`, reused by `Settings/Tabs/LinkedDevices`) · mobile `LinkedDeviceQRCode/LinkedDeviceQRCode.component.tsx` + `screens/LinkedDeviceQRCode`. Stories: Screens/Onboarding → Display QR code (with 2811:2601, 2932:3707 and 880:17427 alongside), · generating, · no community, Settings · Linked devices. The mobile *Linking devices* progress sheet (`879:15508`) is not part of this change.

**In the Settings panel (desktop, #3690).** The sheet was a full-window step of the Link devices modal, and on a short window it ran off the bottom with nothing to scroll. Showing a QR code needs a community, so it is only ever reached from Settings → Linked devices; it is now drawn one level down in that panel, the way QR Code shows the invite (user decision): the designed 220 box, the sentence, and the fine print, and the panel scrolls. Linked devices' back arrow returns to the menu; the QR page's returns to Linked devices. No bar title, no action (the Linked devices panel's own Copy link row copies the link), no Reset QR code. The fine print is the user's copy — *Anyone who has this QR code can link your device and access community history.* — and names no lifetime: the device invite's expiry is checked against the admitting member's own ADMIT timestamp, so a colluding member can backdate past it. Separately, every full-window `ui/Modal` body now scrolls, so a step taller than the window (a long linked-devices list, the camera steps on a short window) stays reachable. Checked in a real browser by `LinkedDevicesQrCode.cy.tsx` and `Modal.scroll.cy.tsx`.
