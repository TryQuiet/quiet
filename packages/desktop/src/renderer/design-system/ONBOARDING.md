...
- **In-app entry is the same full screen (user, 2026-09-13: "it should be full screen right?").** The *Entry points* boards (`879:14680` mobile, `879:19861` desktop) point both the Get started row and the Communities switcher's "Linked devices" row at the one Link devices stage; the in-app state is *With linked* `879:15644`. No sheet/tray variant exists. #3400's bottom-tray context menu (Copy link / QR code / Share + raw link) is deleted; desktop's Settings → Linked devices tab renders the same Link devices content and opens the modal at the chosen step.
- **The rows follow the direction (user, 2026-09-13; not drawn in the file, which shows Display + Scan in every state).** A device that is already in a community *shares*: rows **Display QR code** and **Copy link** (link glyph; copies the same one-time minted link; confirmation is the design's dark check toast `879:15652` reading "Copied", skipped on Android 33+ which confirms itself). A device with no community yet *receives*: rows **Scan QR code** and **Paste link**. The other direction's rows are hidden, not greyed (hiding was one of the two options the user offered; a disabled "Display QR code" on a fresh device only raised the question of why). Intro copy and the Linked devices list are unchanged. Both platforms, PR #3519.

### Sidebar / community home (user request, 2026-09-13)

Source: Quiet Design Library `0j7Nna9zWmfOSNmRmQK1Uh` (unchanged since 2025-04-15), page *Structure & Nav*, section **Desktop and mobile sidebar and titlebar** `6107:21330` (8288×2158; rendered to the session scratchpad `figma/sidebar/section-6107-21330.png`, children + notes in `section-6107-21330.json` / `section-notes.json`). Its designer notes ("Requirements from Holmes", V1 2025 — `6222:13638` desktop, `6220:24045` mobile): *Top bar without search · Channel list with unread badges · Add members button · No Floating Action Button · No dark mode yet · Create channel "+" button · DMs with avatars and badges · Tapped state for all clickable stuff.* State annotations next to the primitives (positions checked against the bounding boxes, 2026-09-13): list item and people-row hover = white 5 % overlay, selected = white 10 %; the "Hover / White 10 %" note sits on *Profile summary* `4716:7702` (the own-user row; its Hover variant carries a "Hover overlay" child at 10 %), not on the people rows. Profile summary renders at 90 % opacity in all variants. The library has no Pressed variant; pressed reuses the Selected overlay.

| use | node | notes |
|---|---|---|
| Desktop sidebar (V1) | `6218:16416` in set `5439:58626` (220×755, purple) | the variant of record; `5439:58627/58760` Dark/Light Full and `5439:66143/66186` Placeholder are later scope |
| Mobile community home (V1) | `6220:10609` in set `5446:76594` (375×764) | `6124:9816` Light/Empty for the no-channels state; `5446:72668/76474` Full are later scope |
| Primitives | Team `4233:12959`, List title `3797:16103`, List item `3797:16113`, List group `3797:16806`, List item--people `4606:16448`, Profile summary `4716:7702`, controls--mac `3797:15261`, Search input `5446:61649` (omitted for V1), FAB `5724:10950` (omitted), drop-line `5582:19302` | |
| 2026 prototype | Community home `2811:2370` (`community-home.png`), switcher `2853:1955`; Draft-6 desktop split view `1430:48044` | same V1 content |

Decisions (2026-09-13): mobile and desktop stay separate components; the mobile list has **no message previews**; "Direct messages" renders as **Users** (Quiet has no DMs) with no badge (no per-user unread exists); Channels "+" only with the create permission; Add members is a row in the card; the mobile title bar's name + caret opens the community menu (the switcher screen is out of scope); no FAB, no search, no own-avatar on mobile; V1 only — no dark variant of the sidebar (the desktop dark theme keeps today's colours until one is designed). Branches: `design/mobile-home`, `design/desktop-sidebar` (both on `design/onboarding-impl`).
