...
- **No "Quiet" header on Get started (user, 2026-09-13):** the prototype's Get started frame (`2811:2550`) and the Dec-2024 desktop *Modal full-window* Get started both carry a title bar reading "Quiet"; it is redundant with the app window on desktop and doesn't feel right on mobile, so the app renders Get started **without a title bar** on both platforms (content starts under the window chrome / at the safe area). The prototype exports still show it; the desktop composition in the click-through drops the bar for this stage (`flow/gen.cjs` `TITLE_BAR`). Every other screen keeps its bar as designed — Join community's "Quiet" bar stays (user, 2026-09-13).
