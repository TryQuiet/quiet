import React, { useCallback, useEffect } from 'react'
import { useTheme } from '@mui/material'
import { ModalName } from '../../../sagas/modals/modals.types'
import { useModal } from '../../hooks'
import UpdateModalComponent from '../../../components/widgets/update/UpdateModalComponent'

import Button from '@mui/material/Button'

import { shell } from 'electron'
import { Site } from '@quiet/common'

const BreakingChangesWarning = () => {
  const modal = useModal(ModalName.breakingChangesWarning)
  const theme = useTheme()

  const title = 'Update required'
  const message =
    'Quiet 9.0 includes important security fixes that are not backwards compatible, so you must re-install Quiet from tryquiet.org and re-create or re-join your community. Once Quiet 9.0 is released, this version (8.x) will stop receiving push notifications and will no longer let new members join unless the owner is online. This version will not receive any further updates or security fixes, so please re-install soon. We apologize for the inconvenience.'

  useEffect(() => {
    modal.handleOpen() // Open modal once per app start
  }, [])

  const updateAction = useCallback(() => {
    shell.openExternal(`${Site.MAIN_PAGE}#Downloads`)
  }, [])

  const updateButton = (
    <Button
      variant='contained'
      size='large'
      color='primary'
      type='submit'
      onClick={updateAction}
      style={{
        height: 55,
        fontSize: '0.9rem',
        backgroundColor: theme.palette.colors.quietBlue,
      }}
      fullWidth
    >
      Install Quiet 9.x
    </Button>
  )

  const dismissButton = (
    <Button
      variant='text'
      size='large'
      color='primary'
      type='submit'
      onClick={modal.handleClose}
      style={{
        height: 'auto',
        padding: 0,
        marginBottom: 0,
        fontSize: '0.9rem',
        color: theme.palette.colors.gray70,
        backgroundColor: theme.palette.colors.white,
      }}
      fullWidth
    >
      Later
    </Button>
  )

  return <UpdateModalComponent {...modal} buttons={[updateButton, dismissButton]} title={title} message={message} />
}

export default BreakingChangesWarning
