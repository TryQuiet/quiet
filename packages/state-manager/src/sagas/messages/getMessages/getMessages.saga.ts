...
import { SocketActions, type MessagesLoadedPayload } from '@quiet/types'

export function* getMessagesSaga(
  socket: Socket,
  action: PayloadAction<ReturnType<typeof messagesActions.getMessages>['payload']>
): Generator {
  const response = yield* apply(socket, socket.emitWithAck, applyEmitParams(SocketActions.GET_MESSAGES, action.payload))
