...
/**
 * Generate root CA cert and add initial Community data to Redux Store
 */
export function* addCommunitySaga(
  socket: Socket,
  action: PayloadAction<ReturnType<typeof communitiesActions.addCommunity>['payload']>
) {
  let CA: null | {
  ...
  if (psk) {
    console.log('addCommunitySaga: saving PSK')
