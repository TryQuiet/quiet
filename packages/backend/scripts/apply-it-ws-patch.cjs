...
const manifest = require('../patch/it/it-ws-6.1.5-source.json')
const patch = path.join(__dirname, '../patch/it/it-ws-6.1.5-source.patch')
const hash = file => crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')

try {
  const requested = path.resolve(process.argv[2] ?? path.join(__dirname, '../node_modules/it-ws'))
  ...
  const packageDirectory = process.argv[2]
    ...
    : path.join(fs.realpathSync(path.join(__dirname, '..')), 'node_modules/it-ws')
  ...
  if (files.every(([file, expected]) => hashes.get(file) === expected.after)) {
    console.log('WebSocket receive lifecycle patch is already applied')
    process.exit(0)
  }
  if (!files.every(([file, expected]) => hashes.get(file) === expected.before)) {
    throw new Error('Unexpected or partially patched WebSocket receive files; refusing to modify this installation')
  }
  ...
  if (!files.every(([file, expected]) => hash(path.join(packageDirectory, file)) === expected.after)) {
    throw new Error('WebSocket receive patch output does not match the verified runtime')
  }
  console.log('Applied WebSocket receive transport lifecycle patch')
} catch (error) {
  console.error(`Cannot apply required WebSocket receive patch: ${error.message}`)
  process.exit(1)
}
