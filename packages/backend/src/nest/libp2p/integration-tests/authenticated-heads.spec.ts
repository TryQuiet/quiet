...
describe('Database heads received before membership synchronization', () => {
  ...
  it.each(['single transport', 'overlapping transport'] as const)(
    'recovers validated profile and history with %s',
    async mode => {
      ...
      const initStores = async (index: number) => {
        ...
        // Require the returning owner to recover from older peers that implement
        // the heads protocol without the new authentication-triggered refresh.
        if (index === 0) module.get(StorageService).attachStoreListeners()
      ...
      let authenticated = 0
      peers[0].on(Libp2pEvents.AUTH_CONNECTED, () => {
        authenticated += 1
      })
      ...
      let delayedHeads = false
      const node = peers[0].libp2pInstance!
      if (mode === 'overlapping transport') {
        const headsGate = new Promise<void>(resolve => {
          releaseHeads = resolve
        })
        const dialProtocol = node.dialProtocol.bind(node)
        jest.spyOn(node, 'dialProtocol').mockImplementation(async (peerId, protocols, options) => {
          const requested = Array.isArray(protocols) ? protocols : [protocols]
          if (!delayedHeads && requested.some(protocol => protocol.endsWith(ownerMessageDb.log.id))) {
            delayedHeads = true
            await raceSignal(headsGate, options?.signal)
          }
          return dialProtocol(peerId, protocols, options)
        })
      }
      releaseSync()
      await waitForExpect(() => expect(authenticated).toBe(1), 10_000)
      if (mode === 'overlapping transport') {
        await waitForExpect(() => expect(delayedHeads).toBe(true), 5_000)
        // Open and authenticate another real Noise/multiplexed transport while
        // the first refresh is delayed. No synthetic auth event or database entry.
        await node.dial(multiaddr(peers[1].localAddress), { force: true })
        await waitForExpect(() => expect(authenticated).toBe(2), 10_000)
      }
      await waitForExpect(async () => {
      ...
      const dial = jest.spyOn(peers[0].libp2pInstance!, 'dialProtocol').mockClear()
