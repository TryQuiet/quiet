...
# Or with random fuzz, seeded, JSON output:
STRESS_FUZZ_RUNS=50 STRESS_FUZZ_SEED=1 \
  STRESS_RESULTS_PATH=/tmp/stress.json \
  PATH=/path/to/node-20/bin:$PATH npm run test-stress-qss
```
...
| File | Focus |
| --- | --- |
| `scenarios/fresh-create-community.stress.spec.ts` | Initial setup. Healthy baseline; high-latency every-round-trip; outage between `VERIFY_CAPTCHA` and `CREATE_COMMUNITY`. Boots a fresh harness per case (QSS rate-limits captcha to one create per socket). |
| `scenarios/fuzz-create-community.stress.spec.ts` | Fuzz sweep: canonical chaos catalog + N randomized profiles (`STRESS_FUZZ_RUNS`). Aggregates failures by error fingerprint with smallest-seed repro. |
| `scenarios/owner-and-member.stress.spec.ts` | Two-client smoke: owner creates community, member joins via QSS-routed AUTH_SYNC, both reach `JOINED`. |
