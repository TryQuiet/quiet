...
export class StorageService extends EventEmitter {
  ...
  public async updatePeerStore() {
    ...
    const inactivePeerIds = new Set(
      endpoints.filter(endpoint => !team.hasDevice(endpoint.deviceId)).map(endpoint => endpoint.peerId)
    )
    ...
    for (const endpoint of activeEndpoints) {
      ...
      if (existingStats) {
        peers[endpoint.peerId] = { ...existingStats, address: multiaddr }
      } else {
