...
const replicatedEvent = async (store: any) => {
  store.events.emit('replicated')
...
describe('CertificatesRequestsStore', () => {
  let ipfs: IPFS
  let orbitdb: OrbitDB
  let store: CertificatesRequestsStore
  ...
  // it('validateUserCsr', async () => {
  //   const orbitdb = await createOrbitDbInstance()
  //   const store = new CertificatesRequestsStore(orbitdb)
  ...
  // TODO
  it.skip('getCsrs - remove old csrs and replace with new for each pubkey', async () => {
    const store = new CertificatesRequestsStore(orbitdb)
    const emitter = new EventEmitter()
    await store.init(emitter)
    ...
    for (const csr of allCsrs) {
      await store.addUserCsr(csr)
    }

    const filteredCsrs = await store.getCsrs()
  ...
  it('replicated event', async () => {
    ...
    await replicatedEvent(store.store)
  ...
  it('2 replicated events - first resolved ', async () => {
    ...
    store.resolveCsrReplicatedPromise(1)
  ...
  it('3 replicated events - two resolved promises ', async () => {
    ...
    await new Promise<void>(resolve => setTimeout(() => resolve(), 500))
    await replicatedEvent(store.store)
    store.resolveCsrReplicatedPromise(2)
    await new Promise<void>(resolve => setTimeout(() => resolve(), 500))
