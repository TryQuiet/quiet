...
export class ChannelsService extends EventEmitter {
  ...
  private validateChannelEntryMetadata(
    ...
    if (decEntry.type === ChannelType.DM) {
  ...
  private validateDmChannelEntry(
    ...
    if (decEntry.public ?? true) {
      this.logger.error('Failed to validate DM channel entry: DMs must be private:', entry.hash)
      return false
    }

    const memberIds = decEntry.memberIds
    if (memberIds == null || memberIds.length === 0) {
      this.logger.error('Failed to validate DM channel entry: memberIds are required:', entry.hash)
      return false
    }

    const uniqueMemberIds = new Set(memberIds)
    if (uniqueMemberIds.size !== memberIds.length) {
      this.logger.error('Failed to validate DM channel entry: memberIds must be unique:', entry.hash)
      return false
    }

    if (!uniqueMemberIds.has(sigAuthor)) {
      this.logger.error('Failed to validate DM channel entry: signer must be a DM participant:', entry.hash, {
        sigAuthor,
        memberIds,
      })
      return false
    ...
    if (decEntry.owner !== sigAuthor) {
      this.logger.error('Failed to validate DM channel entry: owner must match signature author:', entry.hash, {
    ...
    const expectedChannelId = generateDmChannelId(memberIds)
    if (decEntry.id !== expectedChannelId) {
      this.logger.error('Failed to validate DM channel entry: id must match DM participants:', entry.hash, {
        channelId: decEntry.id,
        expectedChannelId,
      })
      return false
    ...
    const chain = this.sigchainService.getActiveChain()
    const expectedRoleName = chain.dms.generateDmRoleName([...memberIds, sigAuthor])
    if (decEntry.roleName !== expectedRoleName) {
      this.logger.error('Failed to validate DM channel entry: roleName must match DM participants:', entry.hash, {
        roleName: decEntry.roleName,
        expectedRoleName,
      })
      return false
    ...
    try {
      chain.users.getUsersById(memberIds)
      const allMembersHaveRole = memberIds.every(memberId => chain.roles.memberHasRole(memberId, expectedRoleName))
      if (!allMembersHaveRole) {
        this.logger.error('Failed to validate DM channel entry: all participants must have the DM role:', entry.hash, {
      ...
      this.logger.error(
        'Failed to validate DM channel entry: all participants must be valid team members:',
        entry.hash,
        {
          memberIds,
          err,
