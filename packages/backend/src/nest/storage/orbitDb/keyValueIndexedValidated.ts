 ...
 */
const Index = ({ directory, validateFn }: { directory?: string; validateFn?: ValidateFn<any> } = {}) => async () => {
  const index = await LevelStorage({ path: directory ?? null, valueEncoding })
  const indexedEntries = await LevelStorage({ path: posixJoin(directory ?? './level', '/_indexedEntries/'), valueEncoding })
    ...
    for await (const entry of log.traverse(null, shoudStopTraverse)) {
      ...
      if (await isNotIndexed(hash)) {
        ...
        if (op === 'PUT' && !keys.has(key) && isValid) {
          keys.add(key)
          await index.put(key as string, entry)
 ...
 * @memberof module:Databases
 */
export const KeyValueIndexedValidated = (validateFn?: ValidateFn<any>) => async ({ ipfs, identity, address, name, access, directory, meta, headsStorage, entryStorage, indexStorage, referencesCount, syncAutomatically, onUpdate }: { ipfs: Helia, identity: Identity, address: string, name: string, access: typeof AccessController, directory: string, meta: Record<string, any>, headsStorage: Storage, entryStorage: Storage, indexStorage: Storage, referencesCount: number, syncAutomatically: boolean, onUpdate: (log: Log, entry: LogEntry) => Promise<void>}) => {
  // Set up the directory for an index
  directory = posixJoin(directory || './orbitdb', `./${address}/_index/`)

  // Set up the index
  const index = await Index({ directory, validateFn })()
