...
import { ComposedStorage, LevelStorage, LRUStorage, Storage } from '@orbitdb/core'
...
const verifySignature = async (signature: string, publicKey: string, data: string | Uint8Array) => {
  if (!signature) {
  ...
  const isValid = (key: PublicKey, msg: Uint8Array, sig: Uint8Array) => key.verify(msg, sig)
 ...
 * @private
 */
const signMessage = async (key: PrivateKey, data: string | Uint8Array): Promise<string> => {
  if (!key) {
 ...
 * @private
 */
const verifyMessage = async (signature: string, publicKey: string, data: string): Promise<boolean> => {
  ...
  if (!cached) {
    ...
    const compare = (cached: Uint8Array, data: Uint8Array | string) => {
      const match = data instanceof Uint8Array ? uint8ArrayCompare(cached, data) === 0 : cached.toString() === data
      return match
    }
 ...
 * @instance
 */
const KeyStore = async ({ path }: { storage?: Storage; path?: string } = {}) => {
  /**
  ...
  const storage = await ComposedStorage(
    ...
    await LevelStorage({ path: path || defaultPath, valueEncoding: 'utf8' })
  )
   ...
   * @instance
   */
  const addKey = async (id: string, key: ComposedKey): Promise<void> => {
    await storage.put('private_' + id, uint8ArrayToString(key.privateKey, 'base16'))
  }
   ...
   * @instance
   */
  const createKey = async (id: string): Promise<PrivateKey> => {
    if (!id) {
   ...
   * @instance
   */
  const getKey = async (id: string): Promise<PrivateKey | undefined> => {
    if (!id) {
    ...
    let storedKey
    try {
      storedKey = Buffer.from(await storage.get('private_' + id), 'hex')
    } catch (e) {
      // ignore ENOENT error
    }
   ...
   * @instance
   */
  const getPublic = (keys: PrivateKey, options: any = {}) => {
...
export { KeyStore, verifyMessage, signMessage }
