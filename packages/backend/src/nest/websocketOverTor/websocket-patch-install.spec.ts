...
const installer = path.join(backendDirectory, 'scripts/apply-it-ws-patch.cjs')
const patchFile = path.join(backendDirectory, 'patch/it/it-ws-6.1.5-source.patch')
const manifestFile = path.join(backendDirectory, 'patch/it/it-ws-6.1.5-source.json')
const installedPackage = path.join(backendDirectory, 'node_modules/it-ws')

/** Exercise installation against copies of the actual pinned package, never shared dependencies. */
describe('Required WebSocket receive runtime patch installation', () => {
  ...
  beforeAll(async () => {
    ...
    temporaryDirectory = await realpath(await mkdtemp(path.join(tmpdir(), 'quiet-it-ws-patch-')))
    ...
    expect(JSON.parse(await readFile(path.join(pristineDirectory, 'package.json'), 'utf8')).version).toBe('6.1.5')
  ...
  it('refuses to patch a shared dependency through the default package symlink', async () => {
    ...
    const fakeScript = path.join(fakeBackend, 'scripts/apply-it-ws-patch.cjs')
    const fakePatchDirectory = path.join(fakeBackend, 'patch/it')
    const packageLink = path.join(fakeBackend, 'node_modules/it-ws')
