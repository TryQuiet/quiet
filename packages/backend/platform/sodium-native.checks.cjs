...
const adapter = require('./sodium-native.cjs')
...
exports.register = function register(test, loadSodium, binding) {
const install = sodium => adapter.install(sodium, binding)
const enable = (sodium, options) => adapter.enable(sodium, {...options, load: () => binding})
...
test('replacement calls the native library; the differential reference remains unchanged', () => {
  let calls = 0
  const counted = new Proxy(Object.fromEntries(Object.getOwnPropertyNames(binding).map(name => [name, binding[name]])), {get(target, name) {
    const value = target[name]
    return typeof value === 'function' ? (...args) => { calls++; return value(...args) } : value
  }})
  const copy = {...original}
  adapter.install(copy, counted)
  calls = 0
  original.crypto_sign_seed_keypair(new Uint8Array(32))
  assert.equal(calls, 0)
  copy.crypto_sign_seed_keypair(new Uint8Array(32))
...
test('small-order and noncanonical signature encodings retain the libsodium rejection policy', () => {
...
test('iOS activates after ready, other platforms preserve sodium, and unsupported capability falls back atomically', async () => {
  ...
  for (const unsupported of [{...original, crypto_box_easy: undefined}, {...original, sodium_version_string: () => 'future-version'}]) {
